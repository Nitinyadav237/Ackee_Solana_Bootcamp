...
impl Calculator {
    ...
    pub fn show_history(&self) -> String {
        ...
        for (i, oper) in self.history.iter().enumerate() {
            let sign = oper.operation_type.get_sign();
            let result = oper.operation_type.perform(oper.first_num, oper.second_num).unwrap();
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64> {
        let prev_oper = self.history.get(operation_index)?.clone();

        let result = prev_oper.operation_type.perform(prev_oper.first_num, prev_oper.second_num)?;

        self.history.push(prev_oper);
        Some(result)
    }
